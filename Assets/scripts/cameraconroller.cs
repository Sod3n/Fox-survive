...
public class cameraconroller : MonoBehaviour
{
	...
	public float distance = 1; // расстояние между камерой и игроком
	public float maxDistance = 3; // максимальное расстояние между камерой и игроком
	public float minDistance = 0.5f; // минимально расстояние между камерой и игроком
	public float scrollStep = 0.1f; // значение изменения расстояние между камерой и игроком за один шаг колессика
	...
	void LateUpdate()
	{
		if (!pause.Pause)
		{
			if (player)
			{
				...
				if (mw < 0)
				{
					if (distance < maxDistance) distance += scrollStep;
				}
				else if (mw > 0)
				{
					if (distance > minDistance) distance -= scrollStep;
				}
			}
            else
            {
